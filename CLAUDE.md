...
- Inputs: the JD can be pasted or uploaded (PDF/DOCX/TXT via `POST /documents/extract-text`, which extracts text without storing the file and fills the editable box); up to **20** supporting items (files + structured projects). Each project has a name, a description and skill/framework tags, and is uploaded as its own supporting doc (`Project: … / Skills and frameworks used: … / Description: …`). Skill tags ("Add skill") and the plain-English "About your skills & knowledge" text are sent together as `extra_text` (max 10k chars). ProfileExtractor trims supplementary text **only when there are more than 5 items**: with ≤5, each is sent whole (40k-char safety ceiling per doc); with more, they share a 120k-character budget split fairly (`fair_limits`: short docs stay whole). `DAILY_UPLOAD_LIMIT` default 100.
...
- Results page in plain language (2026-09-23): a verdict sentence, "What to do next" (links to tabs), one plain sentence per requirement (`plainStatus` in `lib/results.ts`) with evidence under "Where we found it". Every item (score, requirement, suggestion, learning step, interview question) has an **Explain** button → `POST /analyses/{id}/explain {kind, ref}`. The backend builds the facts from the stored analysis (`app/agents/explainer.py`, prompt `explainer.md`; never from client text), one small-model call, cached per item. Interview explanations say what the interviewer is looking for, never a sample answer. Skills typed but not added are added on blur. Matching (scoring v3): a combined skill counts as each named part ("Data structures and algorithms" → dsa, data-structures, algorithms), and when both the requirement and a cited skill are in the taxonomy, only the taxonomy can link them (the LLM can't call "Linear algebra" evidence for "Algorithms"). **Resume-only determinism:** ProfileExtractor (prompt v2) extracts the resume and the supplementary material in two separate cached calls, and the Matcher judges resume evidence and supplementary evidence in two separate cached calls, so editing notes or documents never changes the resume's extraction or the job fit score (only the potential score). Printing always uses the light theme (dark mode is `@media screen` only).
- Feedback round (2026-09-23): **Sign-up** shows a note that a confirmation email will be sent, then a "Confirm your email" screen with steps, an "Open Gmail/Outlook/…" shortcut (`inboxLink`) and a resend button (60 s cooldown); signing in before confirming shows the same screen. **Interview sets are bigger** (prompt v2, `INTERVIEW_SET_VERSION = "2"`): 20–30 technical questions from up to 12 requirements, 10–15 behavioral, 6–10 background from the bank plus personal drill-downs on the student's jobs/degree/certification (`app/rag/data/background_templates.yaml`, MCP tool `get_background_question_templates`, filled in Python), 12–15 per project for up to 5 projects (72 templates). Sets stored by an older version are regenerated on the next "Prepare Me for Interview". **Learning path YouTube links:** ~300 videos in `resources_seed.yaml` from trusted educational channels (relevance-checked, verified via YouTube oEmbed); `get_learning_resources` returns up to 3 other resources then up to 2 videos per skill; every step also links to a YouTube search built in the frontend (never by the LLM).
- Extraction fixes (scoring v4, 2026-09-23): the JDAnalyzer's output is post-processed in Python — a skill/domain/soft-skill requirement that lists several things all wanted ("Git, Docker, Kubernetes, and CI/CD pipelines") is split into one requirement each (`split_requirement`: not for "or" alternatives, known single skills, or when fewer than two parts are taxonomy skills). ProfileExtractor adds a safety net (`missed_skills`): each document is scanned line by line for taxonomy skills the LLM didn't extract (concepts only on short list-like lines), added with the section's context and the verbatim line as evidence. Resume headings like "Elective courses"/"Relevant coursework" are a `coursework` section. `Taxonomy.find_in_text` no longer joins words by accident ("I am" ≠ IAM); "oracle" and "iam" are ambiguous keys. No daily analysis limit.
- Input checks (scoring v5, jd_analyzer prompt v3, 2026-09-23): the JDAnalyzer also returns `is_job_description` (LLM output model `JDAnalysis`); text that isn't a job posting (or yields no requirements) fails the analysis with a clear message (`NotAJobDescriptionError`). A language/tool/database/cloud requirement ("GitHub", "MySQL") only counts a project or job the LLM cites if that item's text names the skill or one implying it (`Matcher._names_it`). The "After adding what you have" score is always shown (header and Improve resume tab), even when it equals the resume score. Test fakes answer a subclass output type from its base's fixture (`FAKE_CHECKS`).
- ATS score + final score (scoring v6, `ATS_VERSION = "1"`, explainer prompt v2, 2026-09-23): the `AtsChecker` agent (no LLM) scores how well an applicant tracking system can read the resume. Layout facts come from the stored resume file (`app/parsing/layout.py`: tables, two-column text, photo/icons, DOCX text boxes, contact only in the header/footer, pages; the pipeline downloads the file via `Repository.download_file` and skips layout checks if it can't). Text facts come from the extracted text (email/phone, written-out LinkedIn/GitHub URLs, standard headings, date styles, personal details like gender/age, word count, unreadable glyphs), plus keywords: each skill/domain requirement counts if the resume names it word for word or by a taxonomy alias (implied skills don't count; must = 3, nice = 1). `app/scoring/ats.py` (pure, 100% covered) turns them into checks with points (`ATS_POINTS` in `weights.py`), `ats_score` = earned / applicable points; its `potential_score` also counts keywords the suggestions add. **Final score = round(0.7 × job fit + 0.3 × ATS)**, shown first everywhere (`headlineScore` falls back to job fit on older analyses); `potential_final_score` uses the potential fit and ATS. Stored in `analyses.ats_score/final_score/potential_final_score` and `analysis_results.ats` (migration `20260923050000_ats_score.sql`). UI: header gauge "Final score" with bars for job fit, ATS and "after adding"; an "ATS check" tab lists checks (problems first) with how to fix, keyword chips and Explain (`kind: "ats" | "ats_check"`, ref = check id); the Markdown export has an ATS section.
